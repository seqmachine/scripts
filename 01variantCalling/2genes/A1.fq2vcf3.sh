...
mkdir -p 0.3mpileUps
mkdir -p 0.1bamFiles
mkdir -p 0.4annotatedVariants

mv *.out.avinput trash
mv *.out.hg19_multianno.vcf trash
mv *.indel.gz 0.2vcfFiles
mv *.snp.gz 0.2vcfFiles
mv *.vcf.gz 0.2vcfFiles
mv *.up 0.3mpileUps
mv *.rmdup 0.1bamFiles
mv *rmdup.bai 0.1bamFiles
mv *.hg19_multianno.txt 0.4annotatedVariants
mv *.BAM.sorted trash
