...
#write.table(cbind(data[,1:4], doc.std), file="180.cnv", sep="\t", row.names=F, col.names=F, quote=F)

write.table(cbind(data[,1:4],apply(doc.std[,1:12], 2, function(x) judge.cnv(x,2.5))),file="cnv.info", sep="\t", quote=F, row.names=F, col.names=F)
