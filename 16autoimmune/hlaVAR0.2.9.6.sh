...
FilterPartialMatchedReads(){
	dir=$1
	
	library=$2

	mkdir -p $dir
	samtools view $name".BAM.sorted.rmdup" | grep "HLA:HLA" | grep "AS:i:" |grep -v "NM:i:0"\
| samtools view -b -T temp/$name"_assignedHLA.fa" > $dir/$name".partial.matched.bam"

#awk -v Q=$mapq '{if($5 < Q) print $0}'  \
#|samtools view -b -T temp/$name"_assignedHLA.fa" > $dir/$name".partial.matched.bam"
	if [ $library == "paired" ]; then
		samtools fastq -N -c 9 --threads $td -1 $dir/$name".partial.matched_1.fastq.gz" -2 $dir/$name".partial.matched_2.fastq.gz" $dir/$name".partial.matched.bam"
	elif [ $library == "single" ]; then
		samtools fastq -N --threads $td $dir/$name".partial.matched.bam" |gzip >$dir/$name".partial.matched.fastq.gz"
	else
...
if [ -s $name".partial.matched"*".fastq.gz" ];then
	...
	FilterPartialMatchedReads $name"_outFQ" $library
	date
...
if [ $library == "single" ]; then
	runOptiTypeSingleEND $name".partial.matched" `pwd`
elif [ $library == "paired" ]; then
	runOptiType $name".partial.matched" `pwd`
else
